...
原版物品通过 `generic.attack_damage` 的修饰符表达武器加值，tooltip 里因此写作
「攻击伤害」。但本 mod 把这部分数值重写成了**基础攻击力**，字样与实际语义不符，
所以武器 tooltip 上那一行会被替换为：

```
基础攻击力   7
```

数值 = 该物品的攻击力修饰符（+6） + 玩家自身的基础攻击力（1），
与原版算法一致（`modif.amount() + entityBase`），因此**数字与改动前完全相同**，
只是标签与归属变成了基础攻击力。配色沿用原版的深绿。

> 实现上通过 `ItemTooltipEvent` 定位并替换原版那一行。
> 由于原版该行的结构是 `literal(" ").append(translatable)`，
> 可翻译内容位于**兄弟节点**而非顶层，定位时必须递归查找。
> 找不到该行时不做任何改动，避免误删其他模组的内容。
