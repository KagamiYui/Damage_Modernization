...
本 mod 在这基础上**追加一行**，说明这把武器折算成基础攻击力时**增加多少**：

```
+6 基础攻击力
```

含义是「装备后基础攻击力 +6」。注意这是武器的**修饰符加值本身**，
不是含玩家自身基础值的总和（钻石剑为 `+6`，而非 `1 + 6 = 7`）。

> 原版的「攻击伤害」行**保持不动**：那是原版属性系统的真实内容，
> 本 mod 只是额外补充一行说明，不干扰其他模组对 `attack_damage` 的读取与显示。
>
> 实现上通过 `ItemTooltipEvent` 追加。定位与去重依赖 `TooltipLines`：
> 原版属性行的结构是 `literal(" ").append(translatable)`，
> 可翻译内容位于**兄弟节点**而非顶层，因此判断时必须递归查找。
